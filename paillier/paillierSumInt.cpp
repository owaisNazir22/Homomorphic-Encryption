...
cout<<"<style>input{ width: 40%; height: 10%; color: brown; border-radius: 60px; background: none; cursor: pointer; border: 2px solid #4caf50; color: white;}</style>"<<endl;
       ...
       cout << h2("D(E(m<sub1</sub>,r<sub>1</sub>).E(m<sub>2</sub>,r<sub>2</sub>) mod n<sup>2</sup>) = m<sub>1</sub>+m<sub>2</sub> mod n.");

clock_t homomorphicStart, homomorphicEnd;
homomorphicStart = clock();
        
string slambda,sn,smu,sn_square,svalue,t1,t2;
mpz_t LAMBDA,N,MU,C,C1,C2,MD,N_sqr,tmp1,tmp2,tmp3,PT,A,B;
mpz_inits(LAMBDA,N,MU,C,C1,C2,MD,N_sqr,tmp1,tmp2,tmp3,PT,A,B,NULL);
...
mpz_set_str(C2,val,16);
     ...
     mpz_mul(C,C1,C2);
     cout << h4("E(m<sub>1</sub>,r<sub>1</sub>).E(m<sub>2</sub>,r<sub>2</sub>) mod n<sup>2</sup>");
     ...
     cout << h4("D(E(m<sub>1</sub>,r<sub>1</sub>).E(m<sub>2</sub>,r<sub>2</sub>) mod n<sup>2</sup>)");
...
file.close();
 ...
 cout << "Size of E(m<sub1</sub>,r<sub>1</sub>).E(m<sub>2</sub>,r<sub>2</sub>) mod n<sup>2</sup> : "<< sizeFile << " bytes" << "</br>" << "</br>";
