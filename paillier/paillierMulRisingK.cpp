...
cout<<"<style>input{ width: 40%; height: 10%; color: brown; border-radius: 60px; background: none; cursor: pointer; border: 2px solid #4caf50; color: white;}</style>"<<endl; 
     cout << head() << endl;
       ...
       cout << h2("D(E(m<sub>1</sub>,r<sub>2</sub>)<sup> k</sup> mod n<sup>2</sup>) = k.m<sub>1</sub> mod n.");
...
string slambda,sn,smu,sn_square,svalue,t1;
mpz_t LAMBDA,N,MU,C,C1,C2,MD,N_sqr,tmp1,tmp2,tmp3,A,PT;
mpz_inits(LAMBDA,N,MU,C,C1,C2,MD,N_sqr,tmp1,tmp2,tmp3,A,PT,NULL);
...
mpz_powm(C,C1,C2,N_sqr);
     ...
     cout << h5("for simiplicity \"k\" has been set as five");
     cout << h4("E(m<sub>1</sub>,r<sub>1</sub>)<sup>k</sup> mod n<sup>2</sup>");
     ...
     cout << h4("D(E(m<sub>1</sub>,r<sub>1</sub>).<sup>k</sup> mod n<sup>2</sup>)");
     cout << "<div>" << mpz_get_str(NULL,10,MD)<< "<div>" "<br>"<<"<br>";
      mpz_mul_ui(PT,A,5);
       cout << h4("k.m<sub>1</sub> mod n");
